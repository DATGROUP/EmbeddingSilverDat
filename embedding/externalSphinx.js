...
var SphinxClass = ( function(){
		...
		function Sphinx(){			
			...
			if (window.postMessage) {
				...
				if (window.addEventListener)  
					window.addEventListener('message', function(msg) { that.onMessage(msg) }, false);
				else if (window.attachEvent) // IE DOM
					window.attachEvent('onmessage', function(msg) { that.onMessage(msg) });
		...
		/** Private Methods */
		/**
		 * The Session id is passed as httponly which means we can not access it from 
    	...
    	*/
		...
		/** inclueds a js file via XMLHttpRequest*/
		...
		Sphinx.prototype.onMessage = function (msg) {
		...
		Sphinx.prototype.cidCallBack = function () {
			if ( sphinx.cid ) {
				...
				if ( sphinx.productVariant ) {
					params += '&productVariant=' + sphinx.productVariant;
				}
		...
		Sphinx.prototype.execute = function ( login, DAF, errorHandler ) {
			...
			if ( this.productVariant ) {
				productVariantParam = '&productVariant=' + this.productVariant;
			}
			
			var that = this;
			LazyLoad.js( sphinx.host + '/post.' + sphinx.servletmapping + '?' + login.toParamString() + '&DAF=' + DAF + '&firstPage=' + this.firstPage + '&lastPage=' + this.lastPage + productVariantParam +
			               '&destination=' + this.destination + style + '&clientLocation=' + encodeURIComponent(location) + '&callback=sphinx.setCId',
			               function() { that.cidCallBack(); } , this, true );            
		};

		/** converts an assosiative array to xml */
